<template>
  <div class="video-box-card">
    ...
    <div class="search">
      <el-select v-model="searchInput.keyOption" style="width: 200px">
        <el-option v-for="item in selectOptions. keyWordOption" :key="item.value" :label="item.label" :value="item.value"/>
      </el-select>
      <el-input v-model="searchInput.keyWord" style="width: 200px" placeholder="请输入关键字"/>
       <el-select v-model="searchInput.dateOption" style="width: 200px">
        <el-option v-for="item in selectOptions.timeOption" :key="item.value" :label="item.label" :value="item.value"/>
      </el-select>
      
      <el-date-picker
        style="width:30%;"
        v-model="searchInput.dateOption"
        type="datetimerange"
        range-separator="-"
      ...
      <el-select v-model="searchInput.selectState" style="width: 200px">
        <el-option v-for="item in selectOptions.stateOption" :key="item.value" :label="item.label" :value="item.value"/>
      </el-select>
      <el-button size="medium" class="button-search"
                 style="background-color:#2DCA93;color: white" @click="getList">搜索
      </el-button>
    </div>
...
import {VideoList, videoList} from '../../../api/api'
  export default {
    name: "VideoList",
    data() {
      return {
    ...
    methods: {
      ...
      getList(){
         var searchObj = {
          timeType:this.searchInput.dateOption,
          beginTime: this.searchInput.dataTime[0] ? this.searchInput.dataTime[0] : '',
          endTime: this.searchInput.dataTime[1] ? this.searchInput.dataTime[1] : '',
          orderStatus: this.searchInput.selectState,
          currentPage: this.queryInfo.PageNum,
          rows: this.queryInfo.PageSize,
        }
       searchObj[['orderId', 'userId', 'userMobile'].includes(this.searchInput.keyOption)?this.searchInput.keyOption:''] = this.searchInput.keyWord ;
        console.log(searchObj)
        videoList(searchObj).then(res=>{
          console.log(res)
           this.tableData = res.list
          this.queryInfo.Total = res.totalCount
          this.queryInfo.TotalPage = res.totalPage
        }).catch(err=>{
          console.log(err)
